  ...
  return (
    <div className="animate-fade-in w-full max-w-6xl mx-auto">
      ...
      {historyItems.length === 0 ? (
        <div className="glass-panel rounded-3xl p-16 text-center flex flex-col items-center justify-center border border-white/5">
          ...
          <button 
            onClick={onBack}
            className="bg-white text-slate-900 px-8 py-3 rounded-xl font-bold hover:bg-violet-200 transition-all shadow-lg hover:shadow-xl"
          >
            Start New Project
          </button>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {historyItems.map((item) => (
            <div key={item.id} className="glass-panel rounded-2xl overflow-hidden group hover:shadow-xl hover:shadow-violet-900/10 transition-all duration-300 border border-white/10 hover:border-violet-500/30">
              <div className="relative h-48 overflow-hidden bg-slate-900">
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-black/80 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-end justify-between p-4">
                  <span className="text-white text-xs font-bold px-2 py-1 bg-black/40 backdrop-blur-md rounded-md border border-white/20">
              ...
              <div className="p-5">
                <div className="flex justify-between items-start mb-3">
                  <h3 className="font-heading font-bold text-lg text-white line-clamp-1" title={item.eventType}>
                    {item.eventType}
                  </h3>
                  <button 
                ...
                <button 
                  ...
                  className="w-full py-3 rounded-xl bg-violet-900/20 text-violet-300 font-bold hover:bg-violet-600 hover:text-white transition-all flex items-center justify-center gap-2 group/btn border border-violet-500/20 hover:border-violet-500"
                >
                  Open Project 
